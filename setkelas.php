...
$kelas = mysqli_query($conn,"select * from tbkelas order by namakelas ASC");
$setkelas = mysqli_query($conn,"select a.nama_siswa, b.namakelas, c.tahun from tbsiswa a, tbkelas b, tbsetkelas c where a.id=c.idsiswa AND b.idkelas=c.idkelas order by b.namakelas, a.nama_siswa ASC");
?>
        ...
        <tr>
            <td>
                <select name="kelas" required>
                    <option vallue="">==pilih kelas==</option>
                    <?php
                    while($baris=mysqli_fetch_array($kelas)){
                        echo "<option value='$baris[idkelas]'>$baris[namakelas]</option>";
                    }
                    ?>
                </select>
            </td>
